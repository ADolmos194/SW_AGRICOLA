...
const props = defineProps(['onEdit', 'onDelete', 'data', 'colums', 'onCol', 'isLoading', 'searchQuery', 'rowPerPage', 'filters', 'onSelected',  'onInput', 'onInputSecondary', 'id', 'iconInput', 'iconInputSecondary', 'idSelect', 'noDeletedFirst'])
...
const inputed = (itemCol) => {
    // idSelectComputed.value = itemCol[ids.value[0]]
...
watchEffect(() => {
    ...
    if (JSON.stringify(dataBackup.value) != JSON.stringify(props.data) || searchQueryComponent.value != props.searchQuery || rowPerPageComponent.value != props.rowPerPage || idSelectComputed.value != props.idSelect){
        dataComponent.value = props.data
        dataBackup.value = props.data
        dataStates.value = props.data
        searchQueryComponent.value = props.searchQuery
        idSelectComputed.value = props.idSelect
    ...
    <VTable class="text-no-wrap">
        <thead>
            <tr>
                ...
                <th v-if="stateEdit || stateDelete || stateInput || stateInputSecondary" scope="col">
                    <span class="text-capitalize text-base font-weight-semibold">OPTIONS</span>
                </th>
        ...
        <tbody v-if="!props.isLoading">
            ...
            <tr v-for="   (item, index)    in    dataTemp   " :key="item.id"
                :style="[item[ids[0]] !== idSelectComputed ? {} : { 'background-color': '#e0f9ff' }]">
                ...
                <td v-if="stateDelete || stateEdit || stateInput || stateInputSecondary" class="text-center" style="width: 5rem;">
                    ...
                    <template v-if="props.noDeletedFirst" >

                        <VBtn v-if="stateDelete && dataTemp.length > 1" icon size="x-small" color="error" variant="text"
                            @click="deleted(item)">
