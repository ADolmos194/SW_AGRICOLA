...
import DepartamentoActivoService from "@/services/servicio_departamento/DepartamentoActivoService";
import ProvinciaService from "@/services/servicio_provincia/ProvinciaService";
...
let id = ref(null);   // ID del PROVINCIA al editar
let searchQuery = ref('')
...
let columns = ref([
    ...
    { key: "departamento", name: "departamento", order: true, search: true, class: 'text-left' },
    {
...
let opcionesDepartamentosActivos = ref([null])
...
const CargarDepartamentosActivos = async () => {
    try {
        const response = await DepartamentoActivoService.getDepartamentosActivos();
        opcionesDepartamentosActivos.value = response
            .map((departamento: { id: any; nombre: any; }) => ({
                value: departamento.id,
                title: departamento.nombre
            }));
    } catch (error) {
        console.error("Error cargar departamento activos:", error);
    }
};
...
const ListaDatos = async () => {
    ...
    try {
        ...
        data.value = response.map((item: { estado: string; }) => {
            return {
                ...item,
...
const nombreRules = computed(() => {
    return [
        ...
        (        v: string | any[]) => (v && v.length >= 2) || 'Mínimo 2 caracteres',
        (        v: string | any[]) => (v && v.length <= 18) || 'Máximo 18 caracteres',
    ];
});

const abreviaturaRules = computed(() => {
    return [
        (        v: any) => !!v || 'Abreviatura es requerida',
        (        v: string | any[]) => (v && v.length >= 2) || 'Mínimo 2 caracteres',
        (        v: string | any[]) => (v && v.length <= 18) || 'Máximo 18 caracteres',
    ];
});
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(provincia.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await ProvinciaService.createProvincia(formData);
        } else {
            response = await ProvinciaService.updateProvincia(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  ProvinciaService.deleteProvincia(item.id);
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVA PROVINCIA' : 'EDITAR PROVINCIA' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="provincia" label="Provincia" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
                                ...
                                <VCol cols="12">
                                    <VSelect v-model="departamento" label="Departamento" :rules="[requiredValidator]"
                                        :items="opcionesDepartamentosActivos"/>
                                </VCol>
...
<route lang="yaml">
    meta:
        action: manage
        subject: general_provincia_provincia
</route>
