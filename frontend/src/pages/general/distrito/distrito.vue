...
import DistritoService from "@/services/servicio_distrito/DistritoService";

import ProvinciaActivaService from '@/services/servicio_provincia/ProvinciaActivaService';
...
let columns = ref([
    ...
    { key: "provincia", name: "provincia", order: true, search: true, class: 'text-left' },
    {
...
let opcionesProvinciasActivas = ref([null])
...
const CargarProvinciasActivas = async () => {
    try {
        const response = await ProvinciaActivaService.getProvinciaActiva();
        opcionesProvinciasActivas.value = response
            .map(provincia => ({
                value: provincia.id,
                title: provincia.nombre
            }));
    } catch (error) {
        titleSnackbar.value = "Error al cargar las provincias activas:", error
...
const onEdit = (item: any) => {
    id.value = item.id;
    distrito.value = item.nombre;
    abreviatura.value = item.abreviatura;
    provincia.value = item.provincia;
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(distrito.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await DistritoService.createDistrito(formData);
        } else {
            response = await DistritoService.updateDistrito(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  DistritoService.deleteDistrito(item.id);
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVO DISTRITO' : 'EDITAR DISTRITO' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="distrito" label="Distrito" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
                                ...
                                <VCol cols="12">
                                    <VSelect v-model="provincia" label="Provincia" :rules="[requiredValidator]"
                                        :items="opcionesProvinciasActivas"/>
                                </VCol>
