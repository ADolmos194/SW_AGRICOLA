...
import PaisService from '@/services/servicio_pais/PaisService'
...
let id = ref(null);   // ID del país al editar
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(pais.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await PaisService.createPais(formData);
        } else {
            response = await PaisService.updatePais(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await PaisService.deletePais(item.id);
...
<template>
    <section>
        ...
        <v-dialog temporary :width="400" location="center" class="scrollable-content" :model-value="dialog"
            ...
            <PerfectScrollbar :options="{ wheelPropagation: false }">
                <VCard flat>
                    ...
                    <div class="d-flex align-center pa-6 pb-1">
                        <h6 class="text-h6">
                            {{ accion == 1 ? 'NUEVO PAíS' : 'EDITAR PAíS' }}
                        </h6>
                    ...
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="pais" label="País" :rules="nombreRules" type="text" />
                                </VCol>
