...
import DepartamentoService from "@/services/servicio_departamento/DepartamentoService";
...
let id = ref(null);   // ID del departamento al editar
let searchQuery = ref('')
...
const onEdit = (item: any) => {
    id.value = item.id;
    departamento.value = item.nombre;
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(departamento.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await DepartamentoService.createDepartamento(formData);
        } else {
            response = await DepartamentoService.updateDepartamento(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  DepartamentoService.deleteDepartamento(item.id);
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVO DEPARTAMENTO' : 'EDITAR DEPARTAMENTO' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="departamento" label="Departamento" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
...
meta:
    action: manage
    subject: general_departamento_departamento
</route>
