...
import CampaniaService from "@/services/servicio_campania/CampaniaService";
...
const ListaDatos = async () => {
    ...
    try {
        const response = await CampaniaService.getCampanias();
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(campania.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await CampaniaService.createCampania(formData);
        } else {
            response = await CampaniaService.updateCampania(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    ...
    try {
        let response: { message_user: any; }
        response = await  CampaniaService.deleteCampania(item.id);
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVA CAMPAÑA' : 'EDITAR CAMPAÑA' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="campania" label="Campaña" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
...
meta:
    action: manage
    subject: agricola_campania_campania
</route>
