...
let columns = ref([
    ...
    { key: "cultivo", name: "cultivo", order: true, search: true, class: 'text-left' },
    {
...
let opcionesCultivosActivos = ref([null])

onMounted(() => {
    ListaDatos();
    CargarEstados();
    CargarCultivosActivos();
});
...
const CargarCultivosActivos = async () => {
    try {
        const response = await CultivoActivoService.getCultivosActivos();
        opcionesCultivosActivos.value = response
            .map(cultivo => ({
                value: cultivo.id,
                title: cultivo.nombre
            }));
    } catch (error) {
        titleSnackbar.value = "Error al cargar los cultivos activos:", error
...
const ListaDatos = async () => {
    ...
    try {
        const response = await VariedadService.getVariedades();
...
const onEdit = (item: any) => {
    id.value = item.id;
    variedad.value = item.nombre;
    abreviatura.value = item.abreviatura;
    cultivo.value = item.cultivo;
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(variedad.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await VariedadService.createVariedad(formData);
        } else {
            response = await VariedadService.updateVariedad(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  VariedadService.deleteVariedad(item.id);
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVA VARIEDAD' : 'EDITAR VARIEDAD' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="variedad" label="Variedad" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
                                ...
                                <VCol cols="12">
                                    <VSelect v-model="cultivo" label="Cultivo" :rules="[requiredValidator]"
                                        :items="opcionesCultivosActivos"/>
                                </VCol>
...
meta:
    action: manage
    subject: agricola_variedad_variedad
</route>
