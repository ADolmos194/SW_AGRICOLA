...
import CultivoService from "@/services/servicio_cultivo/CultivoService";
...
let id = ref(null);   // ID del campaña al editar
let searchQuery = ref('')
...
const ListaDatos = async () => {
    ...
    try {
        const response = await CultivoService.getCultivos();
        ...
        titleSnackbar.value = "Error al cargar los cultivos:", error
    } finally {
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(cultivo.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await CultivoService.createCultivo(formData);
        } else {
            // Editar país existente
            response = await CultivoService.updateCultivo(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  CultivoService.deleteCultivo(item.id);
    ...
    <section>
                    ...
                    <VRow>
                        <VCol cols="12">
                            <DataTableCustom :colums="columns" :data="data" :on-edit="onEdit" :on-delete="onDelete"
                                ...
                                :row-per-page="rowPerPage" :filters="filters" :id-select="id" :label="detalle">
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVO CULTIVO' : 'EDITAR CULTIVO' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="cultivo" label="Cultivo" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
...
meta:
    action: manage
    subject: agricola_cultivo_cultivo
</route>
