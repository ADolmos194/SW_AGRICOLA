...
import CentroService from "@/services/servicio_centro/CentroService";
...
const ListaDatos = async () => {
    ...
    try {
        const response = await CentroService.getCentros();
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(centro.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await CentroService.createCentro(formData);
        } else {
            response = await CentroService.updateCentro(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  CentroService.deleteCentro(item.id);
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="centro" label="Centro" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
...
meta:
    action: manage
    subject: logistica_centro_centro
</route>
