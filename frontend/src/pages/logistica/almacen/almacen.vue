...
import CentrosActivosService from "@/services/servicio_centro/CentrosActivosService";
import AlmacenService from "@/services/servicio_almacen/AlmacenService";
...
let columns = ref([
    ...
    { key: "centro", name: "centro", order: true, search: true, class: 'text-left' },
    {
...
let opcionesCentrosActivos = ref([null])

onMounted(() => {
    ListaDatos();
    CargarEstados();
    CargarCentrosActivos();
});
...
const CargarCentrosActivos = async () => {
    try {
        const response = await CentrosActivosService.getCentrosActivos();
        opcionesCentrosActivos.value = response
            .map(centro => ({
                value: centro.id,
                title: centro.nombre
            }));
    } catch (error) {
        titleSnackbar.value = "Error al cargar las empresas activas:", error
...
const ListaDatos = async () => {
    ...
    try {
        ...
        titleSnackbar.value = "Error al cargar los almacenes:", error
    } finally {
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(almacen.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await AlmacenService.createAlmacen(formData);
        } else {
            response = await AlmacenService.updateAlmacen(id.value, formData);
        }
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  AlmacenService.deleteAlmacen(item.id);
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVO CENTRI' : 'EDITAR CENTRO' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="almacen" label="Almacen" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
                                ...
                                <VCol cols="12">
                                    <VSelect v-model="centro" label="Centro" :rules="[requiredValidator]"
                                        :items="opcionesCentrosActivos"/>
                                </VCol>
...
meta:
    action: manage
    subject: logistica_almacen_almacen
</route>
