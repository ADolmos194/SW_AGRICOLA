...
import GrupoService from "@/services/servicio_grupo/GrupoService";
...
let id = ref(null);   // ID del GRUPO al editar
let searchQuery = ref('')
...
const ListaDatos = async () => {
    ...
    try {
        ...
        titleSnackbar.value = "Error al cargar los grupos:", error
    } finally {
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(grupo.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await GrupoService.createGrupo(formData);
            titleSnackbar.value = response.message_user;  // Mensaje amigable para el usuario
        } else {
            // Editar país existente
            response = await GrupoService.updateGrupo(id.value, formData);
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  GrupoService.deleteGrupo(item.id);
        titleSnackbar.value = response.message_user;
        stateSnackbar.value = 'success';
        isSnackbarVisible.value = false;  // Reiniciar el estado del snackbar
        setTimeout(() => {                 // Asegurarse de que el snackbar vuelva a aparecer
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVO GRUPO   ' : 'EDITAR GRUPO' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="grupo" label="Grupo" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
