...
let columns = ref([
    ...
    { key: "grupo", name: "grupo", order: true, search: true, class: 'text-left' },
    {
...
let opcionesGruposActivos = ref([null])
...
const CargarGruposActivos = async () => {
    try {
        const response = await GrupoActivoService.getGrupoActivas();
        opcionesGruposActivos.value = response
            .map(grupo => ({
                value: grupo.id,
                title: grupo.nombre
            }));
...
const ListaDatos = async () => {
    ...
    try {
        const response = await SubgrupoService.getSubgrupo();
...
const onEdit = (item: any) => {
    id.value = item.id;
    subgrupo.value = item.nombre;
    abreviatura.value = item.abreviatura;
    grupo.value = item.grupo;
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(subgrupo.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await SubgrupoService.createSubgrupo(formData);
            titleSnackbar.value = response.message_user;  // Mensaje amigable para el usuario
        } else {
            // Editar país existente
            response = await SubgrupoService.updateSubgrupo(id.value, formData);
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  SubgrupoService.deleteSubgrupo(item.id);
        titleSnackbar.value = response.message_user;
        stateSnackbar.value = 'success';
        isSnackbarVisible.value = false;  // Reiniciar el estado del snackbar
        setTimeout(() => {                 // Asegurarse de que el snackbar vuelva a aparecer
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVO SUBGRUPO' : 'EDITAR SUBGRUPO' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="subgrupo" label="Subgrupo" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
                                ...
                                <VCol cols="12">
                                    <VSelect v-model="grupo" label="Grupo" :rules="[requiredValidator]"
                                        :items="opcionesGruposActivos"/>
                                </VCol>
...
meta:
    action: manage
    subject: logistica_subgrupo_subgrupo
</route>
