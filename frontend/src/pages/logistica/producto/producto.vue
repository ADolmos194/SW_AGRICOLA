...
import GrupoActivoService from "@/services/servicio_grupo/GrupoActivoService";
import SubgrupoService from "@/services/servicio_subgrupo/SubgrupoService";
import SubgrupoActivoService from "@/services/servicio_subgrupo/SubgrupoActivoService";
import ProductoService from "@/services/servicio_producto/ProductoService";
...
let columns = ref([
    ...
    { key: "subgrupo", name: "subgrupo", order: true, search: true, class: 'text-left' },
    {
...
let opcionesSubgruposActivos = ref([null])

onMounted(() => {
    ListaDatos();
    CargarEstados();
    CargarSubgruposActivos();
});
...
const CargarEstados = async () => {
    try {
        ...
        titleSnackbar.value = "Error al cargar las estados:", error
    }
};

const CargarSubgruposActivos = async () => {
    try {
        const response = await SubgrupoActivoService.getSubgruposActivos();
        opcionesSubgruposActivos.value = response
            .map(subgrupo => ({
                value: subgrupo.id,
                title: subgrupo.nombre
            }));
    } catch (error) {
        titleSnackbar.value = "Error al cargar las grupos activos:", error
...
const ListaDatos = async () => {
    ...
    try {
        ...
        titleSnackbar.value = "Error al cargar los subgrupos:", error
    } finally {
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(producto.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await ProductoService.createProducto(formData);
            titleSnackbar.value = response.message_user;  // Mensaje amigable para el usuario
        } else {
            // Editar país existente
            response = await ProductoService.updateProducto(id.value, formData);
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  ProductoService.deleteProducto(item.id);
        titleSnackbar.value = response.message_user;
        stateSnackbar.value = 'success';
        isSnackbarVisible.value = false;  // Reiniciar el estado del snackbar
        setTimeout(() => {                 // Asegurarse de que el snackbar vuelva a aparecer
    ...
    <section>
        ...
        <VNavigationDrawer temporary :width="400" location="end" class="scrollable-content" :model-value="dialog"
            ...
            <div class="d-flex align-center pa-6 pb-1">
                <h6 class="text-h6">
                    {{ accion == 1 ? 'NUEVO PRODUCTO' : 'EDITAR PRODUCTO' }}
                </h6>
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="producto" label="Producto" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
                                ...
                                <VCol cols="12">
                                    <VSelect v-model="subgrupo" label="Subgrupo" :rules="[requiredValidator]"
                                        :items="opcionesSubgruposActivos"/>
                                </VCol>
...
meta:
    action: manage
    subject: logistica_producto_producto
</route>
