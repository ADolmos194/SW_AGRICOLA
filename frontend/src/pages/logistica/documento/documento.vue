...
import DocumentoService from "@/services/servicio_documento/DocumentoService";
...
const ListaDatos = async () => {
    ...
    try {
        const response = await DocumentoService.getDocumentos();
        ...
        console.error("Error al cargar las tipo documento:", error);
    } finally {
...
const onSubmit = async () => {
    const nombreValidacion = nombreRules.value.every(rule => rule(documento.value) === true);
    ...
    try {
        ...
        if (accion.value === 1) {
            response = await DocumentoService.createDocumento(formData);
            titleSnackbar.value = response.message_user;  // Mensaje amigable para el usuario
        } else {
            // Editar país existente
            response = await DocumentoService.updateDocumento(id.value, formData);
...
const onDelete = async (item: any) => {
    try {
        let response: { message_user: any; }
        response = await  DocumentoService.deleteDocumento(item.id);
        titleSnackbar.value = response.message_user;
        stateSnackbar.value = 'success';
        isSnackbarVisible.value = false;  // Reiniciar el estado del snackbar
        setTimeout(() => {                 // Asegurarse de que el snackbar vuelva a aparecer
                ...
                <VCard flat>
                    <VCardText>
                        ...
                        <VForm ref="refForm" @submit.prevent="onSubmit">
                            <VRow>
                                <VCol cols="12">
                                    <VTextField v-model="documento" label="Documento" :rules="nombreRules" 
                                    type="text" />
                                </VCol>
                                <VCol cols="12">
...
meta:
    action: manage
    subject: logistica_documento_documento
</route>
